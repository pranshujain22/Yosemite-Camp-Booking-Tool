...
def run(driver):
    try:
        ...
        camp_site = driver.find_element(By.XPATH,
                                        f'//button[contains(@aria-label, "{config["date"]} - Site Camp 4 is available")]')
        ...
        group_size.send_keys(config["group_size"])
        ...
        for index in range(1, config["group_size"]):
            wait = WebDriverWait(driver, 10)
            ...
            first_name.send_keys(config['group_details'][index]['first_name'])

            last_name = driver.find_element(By.ID, f'last_name-{booking_id}-{index}')
            last_name.send_keys(config['group_details'][index]['last_name'])
            ...
            postal_code.send_keys(config['group_details'][index]['postal_code'])
        ...
        driver.execute_script("arguments[0].scrollIntoView(true);", first_name)
        driver.execute_script("arguments[0].value = '" + config['group_details'][-1]['first_name'] + "';", first_name)
        ...
        driver.execute_script("arguments[0].value = '" + config['group_details'][-1]['last_name'] + "';", last_name)
        ...
        driver.execute_script(f"arguments[0].value = '{config['group_details'][-1]['postal_code']}';", postal_code)
